...
export default function TeacherAttendanceReport() {
  ...
  const loadRecent = async () => {
    try {
      ...
      const url = `${base}${API_CONFIG.ENDPOINTS.ADMIN}/recent-attendance?limit=50`;
    ...
    >
      ...
      {records.length === 0 ? (
        ...
        records.map(r => (
          ...
          >
            <View style={{ flex: 1 }}>
              <Text style={styles.name}>{r.userName || 'Unknown User'}</Text>
              <Text style={styles.meta}>
                {r.type.toUpperCase()} • {formatTime(r.timestamp)}
              </Text>
            </View>
            <Ionicons
